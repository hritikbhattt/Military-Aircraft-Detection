...
```
Python 3.10+ and PyTorch 2.0+ are required (the codebase uses modern union-type syntax, e.g. `int | None`). A CUDA-capable GPU is strongly recommended for training (this project's model was trained on a Kaggle-hosted Tesla T4). CPU is sufficient for inference.  
...
![Per-class AP](outputs/eval/per_class_ap.png)
...
## Known Issues
- **Custom metrics module reports a lower mAP50 than Ultralytics' official validator** (0.588 vs 0.654 at low confidence threshold, 0.540 vs 0.654 at the default 0.25 threshold). Investigated: confidence-threshold filtering in `evaluate.py`'s prediction step accounts for roughly 60% of the gap (verified by lowering `--conf` to 0.001, which raised the custom mAP50 from 0.540 to 0.588). The remaining gap is attributed to differences between the custom module's 101-point AP interpolation and Ultralytics' internal AP computation - both are valid but non-identical conventions. The Ultralytics-reported numbers in the Results table above are the primary, trusted metrics.
- Previously, `.github/workflows/ci.yml` was failing due to a class-schema mismatch after the 36-to-103 class migration (stale imports and a `val`/`validation` folder-naming inconsistency across three files). All issues identified and fixed - CI is now passing.
- Previously, `scripts/evaluate.py`'s custom per-class report crashed on a type error when reading `data.yaml`'s `names` field. Fixed by normalizing the parsed dict to an ordered list before use.
